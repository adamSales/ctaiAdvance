...
trptVars <- names(simList)[sapply(simList,ncol)%in%c(NULL,1:20)]
#trtpVars <- setdiff(trptVars,highD)

for(vvv in trptVars) traceplotSL(simList,param=vvv,ask=TRUE)

summ[do.call('c',lapply(trptVars, function(varb) grep(varb,rownames(summ)))),]

par(mfrow=c(3,4))
for(varb in setdiff(names(simList),trptVars)){
    print(varb)
...
for(i in 1:ncol(trtEff)) trtEff[,i] <- trtEff[,i]+simList$tau
...
plotEffect <- function(simList,summ){
    ...
    sapply(1:nrow(studEff), function(rr) lines(sort(studEff[rr,]),trtEff[rr,order(studEff[rr,])],
                                               col=adjustcolor(simList$chain[rr],.1)))
