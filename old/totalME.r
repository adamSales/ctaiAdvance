...
X <- model.matrix(~race+sex+spec+state,data=dat)[,-1]
X <- scale(X)
ncovar <- ncol(X)
...
xirt <- dat$xirtOrig
studentX <- sid[!is.na(xirt)]
sem <- dat$xirt_sem[!is.na(xirt)]
xirt <- xirt[!is.na(xirt)]
n.x.obs <- length(xirt)


jagsDat <- list('nsecWorked','grad','studentM','section','X','teacher','Z','Xm','school','nstud','Y','ntch','nscl','nsec','unit','nunit','ncovar','covX','pair','npair','xirt','sem','studentX','n.x.obs')

params <- c('studEff','secEff','alphaGrad','betaU','teacherEffU','Ynew','sig2Y','useEff','trtEff','betaY','teacherEffY','Ustd','a','b','sig2TchU','sig2TchY','sig2SclU','sig2SclY','sig2Sec','sig2Un','sig2U','pairEffect','sdSeff','avgSeff','a0','a1','a2','a3','b0','b1','b2','b3','betaPretest','teacherEffPretest','schoolEffPretest','by.pretest','by.pretest2','bu.pretest','bu.pretest2','sig2Pretest','sig2TchX','sig2SclX')
...
print(Sys.time())
realModels$lin <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodME.bug',n.chains=4,n.iter=10000,n.thin=5)
print(Sys.time())

realModels$rob <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodRobME.bug',n.chains=4,n.iter=10000,n.thin=5)

save(list=ls(),file=paste0('~/Google Drive/CTmodels/realModels',Sys.Date(),'ME.RData'));rm(realModels);gc()
...
xirt <- dat$xirtOrig
sem <- dat$xirt_sem
pretest.obs <- ifelse(is.na(dat$xirtOrig),0,1)


noEffModels <- list()

print(Sys.time())
noEffModels$lin <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodME.bug',n.chains=4,n.iter=10000,n.thin=4)
save(list=ls(),file=paste0('~/Google Drive/CTmodels/noEffModels',Sys.Date(),'ME.RData'));rm(noEffModels);gc()
...
print(Sys.time())
constEffModels$lin <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodME.bug',n.chains=4,n.iter=10000,n.thin=4)
save(list=ls(),file=paste0('~/Google Drive/CTmodels/constEffModels',Sys.Date(),'ME.RData'));rm(constEffModels);gc()
...
print(Sys.time())
linEffModels$lin <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodME.bug',n.chains=4,n.iter=4000,n.thin=4)

save(list=ls(),file=paste0('~/Google Drive/CTmodels/linEffModels',Sys.Date(),'ME.RData'));rm(linEffModels);gc()
...
print(Sys.time())
quadEffModels$lin <- jags.parallel(jagsDat,parameters=params,model.file='src/psmodME.bug',n.chains=4,n.iter=4000,n.thin=4)
print(Sys.time())

save(list=ls(),file=paste0('~/Google Drive/CTmodels/quadEffModels',Sys.Date(),'ME.RData'));rm(quadEffModels);gc()
